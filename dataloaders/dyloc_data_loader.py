        ...
        for i, vid_clip_name in enumerate(os.listdir(stim_dir_path)):
            ...
            while vid.isOpened():
                ...
                if ret:
                    ...
                    frame = cv2.resize(
                        frame, dsize=(64, 64), interpolation=cv2.INTER_CUBIC
                    )
...
class DylocDataloader(PsychDataloader):
    ...
    def get_batch(self, num_vids_in_batch=10):
        ...
        for cond_idx, condition in enumerate(condition_names):
            ...
            file_names = [
                f for f in os.listdir(condition_path) if ("exp" in f and f[0] != ".")
            ]
            ...
            for idx, file in enumerate(batch_files):
                ...
                imgs = sorted(
                    [
                        im
                        for im in os.listdir(vid_path)
                        if (".png" in im and im[0] != ".")
                    ]
                )
                for frame, img_path in enumerate(imgs):
                    ...
                    img = img.reshape(
                        (3, self.exp_image_size[0], self.exp_image_size[1])
                    )
...
if __name__ == "__main__":
    make_downseampled_stim(
        "/Users/loggiasr/Projects/fmri/monkey_fmri/MTurk1/stimuli/dyloc_stimuli",
