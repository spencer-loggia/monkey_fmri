...
def interactive_program():
    ...
    if existing_project:
        ...
        _create_dir_if_needed(os.path.dirname(project_def['project_root']),
                              os.path.basename(project_def['project_root']))
        project_def['subjects'] = _str_list_input("Enter subject names included in this project")
        project_def['subject_data_roots'] = dict()
        for subject in project_def['subjects']:
            project_def['subject_data_roots'][subject] = _create_dir_if_needed(project_def['project_root'], subject)
            for sub_dir in ['sessions', 'mri', 'surf', 'analysis']:
                _create_dir_if_needed(project_def['subject_data_roots'][subject], sub_dir)
    ...
    while choice != 7:
        choice = _select_option_input(['Import new dicoms',
