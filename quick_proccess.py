...
def interactive_program():
    print(
        "Running Interactive Intrasubject fMRI Preprocessing and First Level Analysis Code. \n"
    ...
    print(
        " You can either load a existing project state or create a new one. Existing projects will by default preform "
        " Analysis using all available data, and bring new data added up to data "
    )

    existing_project = _bool_input("do you want to open an existing project? ")

    if existing_project:
        project_json = _dir_input("Enter path to project json file")
        with open(project_json, "r") as f:
            project_def = json.load(f)
        print("loaded project", project_def["name"])
    else:
        print("creating new project...")
        project_def = dict()
        project_def["name"] = input("Name of project: ")
        project_def["project_root"] = input("Path to root of project directory: ")
        _create_dir_if_needed(
            os.path.dirname(project_def["project_root"]),
            os.path.basename(project_def["project_root"]),
        )
        project_def["subjects"] = _str_list_input(
            "Enter subject names included in this project"
        )
        project_def["subject_data_roots"] = dict()
        for subject in project_def["subjects"]:
            project_def["subject_data_roots"][subject] = _create_dir_if_needed(
                project_def["project_root"], subject
            )
            for sub_dir in ["sessions", "mri", "surf", "analysis"]:
                _create_dir_if_needed(
                    project_def["subject_data_roots"][subject], sub_dir
                )
        project_def["paradigm_def_jsons"] = dict()

    print("Loaded project.")
    choice = -1

    while choice != 7:
        choice = _select_option_input(
            [
                "Import new dicoms",
                "Preprocess some runs",
                "generate registration transforms for some runs",
                "mask functional data",
                "Generate contrast volumes using some runs",
                "project contrasts to anatomical space",
                "generate contrast surfaces",
                "exit",
            ]
        )

        if choice == 0:
            print("which subject are these runs from? ")
            subject = _select_option_input(project_def["subjects"])
            dicom_dir = _dir_input(
                "Enter directory containing DICOMS from scans we are trying to analyze"
            )
            run_numbers = _int_list_input(
                "Enter whitespace separated list of valid epi run IMA numbers. (e.g. '2 4 5')"
            )
            f_dir = os.path.join(project_def["project_root"][subject], "sessions")
        ...
        if check:
            ...
            SOURCE = check_time_series_length(
        ...
        t1_mask_name = ""
        print(
            "Anatomical data is assumed to be located in [root]/mri, in this case",
            ana_dir,
        )
        ...
        print(
            "The following initial preprocessing steps will now be preformed. \n"
        ...
        # save masked t1
        t1_masked = _apply_binary_mask_3D(
            in_img=nib.load(ds_t1_path), mask=nib.load(ds_t1_mask_path)
        )
        ...
        if do_init_preprocess:
            ...
            SOURCE = motion_correction(
                SOURCE, fname=process_head, abs_threshold=1.0, var_threshold=0.3
            )
            process_head = "moco.nii.gz"
            create_low_res_anatomical(
                source_dir=ana_dir, fname=t1_name, factor=2.0, output=ds_t1_path
            )
            create_low_res_anatomical(
                source_dir=ana_dir,
                fname=t1_mask_name,
            ...
            process_head = input(
                "What is the basename if the function inputs to use going forward?"
            )

        print(
            "Next step is to preform an initial manual registration from function to anatomical in ITK-Snap. \n"
        ...
        if do_quick_reg:
            ...
            if do_man_reg:
                ...
                avg_func = average_functional_data(
                    SOURCE,
                    output=os.path.join(sessDir, average_f),
                    fname=process_head,
                    through_time=True,
                )
                res = manual_itksnap_registration(
                    [sessDir], fname=average_f, template_file=ds_t1_path
                )
                print(res)
                ...
                print(
                    "Now the anatomical brain mask must be applied to the manually registered functional data files."
                )
                if _bool_input(
                    "Do you wish to apply the brain mask to average functional ?"
                ):
                    f_masked = _apply_binary_mask_3D(
                        in_img=nib.load(os.path.join(sessDir, average_f_man_reg)),
                        mask=nib.load(ds_t1_mask_path),
                    )
                    average_f_man_reg_stripped = "avg_f_stripped.nii"
                    nib.save(
                        f_masked, os.path.join(sessDir, average_f_man_reg_stripped)
                    )

                print("Now we can automatically register the stripped epis to the t1")
            else:
                man_transform_path = _dir_input(
                    "Enter path to grass spatial transform (i.e. the one that would have been "
                    "defined manually)"
                )
            if _bool_input("Do you want to do automatic coregistration?"):
                ...
                fine_transform_path, fine_transform_inverse_path = antsCoReg(
                    ds_t1_masked_path,
                ...
                fine_transform_path = _dir_input(
                    "Enter path to fined grained forward transform (should be an h5 file "
                    "defined using ants coregistration: "
                )
                fine_transform_inverse_path = _dir_input(
                    "Enter path to inverse fine grained transform (h5 file): "
                )

            print(
                "We can use the inverse of the registration transforms to create a brain mask in the native functional "
            ...
            if mask_functional:
                if not average_f:
                    average_f = _dir_input(
                        "specify path to 3d functional average unstripped in native space: "
                    )
                ...
                to_invert = [
                    False,
                    True,
                ]  # manual transform affine matrix is invertable by applywarp,
            ...
            #         p.starmap(antsApplyTransforms, reg_args)
                ...
                process_head = input(
                    "Enter file name for functional data process head: "
                )
            #
        ...
        if _bool_input("Continue to first level analysis?? "):
            ...
            print(
                "To complete all steps in the First Level Analysis we need the following files."
            )
            print(
                "- functional data registered to a t1 \n"
            ...
            if _bool_input("do you already have a paradigm definition json file?"):
                para_json = _dir_input(
                    "Enter the path to the existing paradigm order definition json. "
                )
                ...
                for i in range(num_conditions):
                    condition_map[str(i)] = input(
                        "description for condition #" + str(i)
                    )
            ...
            if interactive_ima_map:
                ...
                save_path = os.path.join(
                    os.path.dirname(SOURCE[0]), "ima_order_map.json"
                )
                with open(save_path, "w") as f:
                    json.dump(ima_order_map, f)
                print(
                    "Successfully saved ima to order number mapping json at", save_path
                )
            else:
            ...
            while add_contrast:
                ...
                if len(contrast) != para_def_dict["num_conditions"]:
                    print(
                        "contrast vector must have length equal to the number of conditions."
                    )
                    continue
            ...
            for run_dir in SOURCE:
                ...
                design_matrix = design_matrix_from_order_def(
                    block_length, num_blocks, num_conditions, order, convolve=False
                )
                design_matrices.append(design_matrix)
            contrast_imgs = intra_subject_contrast(
                SOURCE,
                design_matrices,
                contrasts,
                contrast_descriptions,
            ...
            contrast_paths = [
                os.path.join(analysis_dir, s + "_contrast.nii")
                for s in contrast_descriptions
            ]
        ...
        if _bool_input("generate contrast surfaces? "):
            ...
            if not ref_img:
                ref_img = _dir_input(
                    "Enter path to target registration image in anatomical space (ideally an already"
            ...
            print(
                "next step is to generate contrast surfaces... assuming surfaces are located in anatomical dir and called "
                "rh.white, lh.white, rh.inflated, lh.inflated"
            )

            generate_surfaces = _bool_input(
                "Do you want to generate contrast surfaces?"
            )
            ...
            if generate_surfaces:
                ...
                for contrast_path in contrast_paths:
                    for hemi in ["rh", "lh"]:
                        ...
                        print(root)
                        create_contrast_surface(
                            os.path.join(root, "surf", hemi + ".white"),
                            contrast_path,
                            ds_t1_path,
                            t1_path,
                            hemi=hemi,
