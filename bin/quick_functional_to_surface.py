...
import sys
if len(sys.argv) < 3:
    print('Please supply (1) subject name, (2) path to directory containing functional space niftis, (3) the kind of contrasts, choose from [snr, scp, scd, any], and (4) functional img smoothing in mm (0 for none)')
else:
    import os
    ...
    if __name__ == "__main__":
        ...
        if kind == 'snr':
            nii_paths = [f for f in os.listdir(nii_dir) if ".nii.gz" in f and "SNR" in f or "probe" in f and "~" not in f]
        elif kind == 'scd':
            nii_paths = [f for f in os.listdir(nii_dir) if ".nii.gz" in f and "minus" in f or "probe" in f and "~" not in f]
        elif kind == 'scp':
            nii_paths = [f for f in os.listdir(nii_dir) if ".nii.gz" in f and "minus" in f or "scp" in f and "~" not in f]
        elif kind == 'any':
            nii_paths = [f for f in os.listdir(nii_dir) if ".nii.gz" in f and "~" not in f and "itk" not in f and "3d" not in f and "Warped" not in f]
        else:
        	print('any?')
        ...
        for i, path in enumerate(nii_paths):
            ...
            if smooth != 0:
                ...
                print('Smoothing will decrease noise but lose spatial detail. You are using full-width half-maximum smoothing over ', smooth, ' mm')
                smooth_name = str(smooth).split('.')[0] + '_' + str(smooth).split('.')[1] + 'mm'
                path = str(path).split('.')[0]+'_smoothed_'+str(smooth_name)+'.nii.gz'
        ...
        func_to_anat = apply_warp(nii_paths, volume_in_target_space, forward_gross_transform_path, fine_transform_path)
        hemis = ['lh', 'rh']
        
        anat_nii_paths = [f for f in os.listdir(nii_dir) if ".nii.gz" in f and "reg" in f and "~" not in f]
