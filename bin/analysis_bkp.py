...
def design_matrix_from_run_list(
    ...
    fir_dm = _make_nl_dm(
        frames, fir_time_df, "fir", delay_periods=[1], stim_min_onset=-32
    )
    hrf_dm = _make_nl_dm(
        frames, hrf_time_df, hrf, delay_periods=[0], stim_min_onset=-32
    )
...
def nilearn_contrasts(
    ...
    contrast_matrices = [
        [] for _ in range(contrast_matrix.shape[1] + 1)
    ]  # room for auto background contrast
