...
if len(sys.argv) < 9:
    print(
        "Please supply subject name, path to session directory, SNR estimated on scan day, session MION dosage, number of used runs, receive coils (t for turquoise, o for orange, n for nif), path to roi directory, and True if you want measurements in whole brain AND rois"
    )
    ...
    scan_snr_est = float(
        sys.argv[3]
    )  # SNR estimated on scan day, should be in scan notes
    ...
    background_mask_path = os.path.join(
        "/home/ssbeast/Projects/SS/monkey_fmri/MTurk1_12TB/subjects",
        subject,
        "mri/functional_target_background_mask.nii.gz",
    )
    ...
    roi_mask_paths = [
        ...
        if ".nii.gz" in f and "~" not in f and "reg" not in f
    ]  # don't grab the "~" backup files
    ...
    if temp_arg:  # len(glob.glob(os.path.join(session_dir, '*/reg_moco.nii.gz')))>0:
        ...
        for root, directory, file in os.walk(session_dir, topdown=True):
            for f in file:
                if f == "epi_masked.nii.gz":
                    ...
                    to_mask = to_mask[
                        :, :, :, 0
                    ]  # discard time dimension by just taking mask at first time point
                    binary_mask = np.where(to_mask > 0.0, 1.0, 0.0)

                    binary_img = nib.Nifti1Image(binary_mask, nib.load(fullpath).affine)
                    binary_img_outpath = os.path.join(
                        os.path.dirname(fullpath), "epi_masked_binary.nii.gz"
                    )
        ...
        for root, directory, file in os.walk(session_dir, topdown=True):
            for f in file:
                if f == fname:  # load ima reg_moco file
                    ...
                    t_epi_mean = epi.mean(
                        axis=3
                    )  # mean signal across time at each voxel; shape (128, 100, 42)
                    t_epi_std = epi.std(
                        axis=3
                    )  # standard dev signal across time at each voxel; shape (128, 100, 42)

                    epi_mask_path = os.path.join(
                        root, "epi_masked_binary.nii.gz"
                    )  # grab binary brain mask for ima
                    ...
                    if in_roi:
                        for j, mask_path in enumerate(roi_mask_paths):
                            ...
                            roi_dat = nib.load(
                                os.path.join(rois_dir, mask_path)
                            ).get_fdata()
        ...
        # create session df
        session_SNR_report = pd.DataFrame(
            {
                "ima": ima,
                "scanner_SNR": scanner_snr,
                "MION": mion_dose,
                "num_runs": num_good_runs,
                "receive_coils": receive_coils,
                "whole_brain_tSNR": sesh_tSNR_val,
                "whole_brain_tBSNR": sesh_tBSNR_val,
            }
        )
        # session_SNR_report.to_csv(os.path.join(sesh, 'SNR_report.csv'))

        if in_roi:
            roi_df_tSNR = pd.DataFrame(
                {"ima": roi_ima, "roi": roi_names_tSNR, "tSNR": roi_tSNR_avgs}
            )
            roi_df_wide_tSNR = roi_df_tSNR.pivot(
                index="ima", columns="roi", values="tSNR"
            )
            roi_df_wide_tSNR.loc["Avg"] = roi_df_wide_tSNR.mean()
            roi_df_wide_tSNR = roi_df_wide_tSNR.reset_index()

            roi_df_tBSNR = pd.DataFrame(
                {"ima": roi_ima, "roi": roi_names_tBSNR, "tBSNR": roi_tBSNR_avgs}
            )
            roi_df_wide_tBSNR = roi_df_tBSNR.pivot(
                index="ima", columns="roi", values="tBSNR"
            )
            roi_df_wide_tBSNR.loc["Avg"] = roi_df_wide_tBSNR.mean()
            roi_df_wide_tBSNR = roi_df_wide_tBSNR.reset_index()

            full_SNR_report = session_SNR_report.merge(
                roi_df_wide_tSNR, how="inner", on="ima"
            ).merge(roi_df_wide_tBSNR, how="inner", on="ima")
        else:
