...
def include_patterns(*patterns):
    ...
    def _ignore_patterns(path, all_names):
        ...
        dir_names = (
            name for name in all_names if os.path.isdir(os.path.join(path, name))
        )
...
def get_images(null, *argv):
    ...
    if from_dicom:
        dicom_dir = input_control.dir_input(
            "Enter directory contraining DICOMS of images we want to look at"
        )
...
def get_epis(*argv):
    session_id = argv[0]
    subj_root, project_root = _env_setup()
    from_dicom = input_control.bool_input(
        "load new session dicoms? (otherwise must import niftis from exp)"
    )
    f_dir = preprocess._create_dir_if_needed(subj_root, "sessions")
    if from_dicom:
        dicom_dir = input_control.dir_input(
            "Enter directory containing DICOMS from scans we are trying to analyze"
        )
        ...
        print(
            "Created the following functional run directories: \n ",
            SOURCE,
        ...
        nifti_name = input(
            "Enter name of nifti files to transfer (should be unprocessed versions"
        )
        sess_target_dir = os.path.join(f_dir, str(session_id))
        if not os.path.samefile(nifti_source, sess_target_dir):
            shutil.copytree(
                nifti_source, sess_target_dir, ignore=include_patterns(nifti_name)
            )
        else:
            preprocess._create_dir_if_needed(f_dir, str(session_id))
        SOURCE = [
            os.path.relpath(os.path.join(sess_target_dir, f), project_root)
            for f in os.listdir(sess_target_dir)
            if f.isnumeric()
        ]
    ...
    if check:
        ...
        SOURCE = preprocess.check_time_series_length(
            SOURCE, fname="f.nii.gz", expected_length=expected
        )
...
def get_functional_target():
    ...
    path = input_control.dir_input(
        "Enter path to functional data representative image (should be masked): "
    )
    if ".nii.gz" in path:
...
def get_fixation_csv(source):
    subj_root, project_root = _env_setup()
    path = input_control.dir_input(
        "Path to fixation csv file (length trs, header with IMAs)"
    )
    out_paths = []
...
def dilate_mask(inpath, outpath=None):
    ...
    cmd = "fslmaths %s -bin -mul -1 -add 1 -kernel boxv 7 -ero -mul -1 -add 1 %s" % (
        inpath,
        outpath,
    )
    call(cmd, shell=True)
...
def downsample_anatomical(
    inpath, factor=[2, 2, 2], out_dir=None, affine_scale=None, resample="interpolate"
):
    """
    ...
    preprocess.create_low_res_anatomical(
        source_dir,
        name,
        output,
        factor=factor,
...
def downsample_vol_rois(
    roi_dict, ds_roi_dict, ds_t1, affine_scale=None, resample="nearest", output_dir=None
):
    ...
    if output_dir is None:
        output_dir = os.path.relpath(
            preprocess._create_dir_if_needed(
                os.path.join(subj_root, "rois", roi_set_name), "ds_roi_vols"
            ),
            project_root,
        )
...
def downsample_vol_rois_cmdline_wrap(
    dir, factor=[2, 2, 2], affine_scale=None, resample="nearest", output_dir=None
):
    """
...
def coreg_wrapper(source_space_vol_path, target_space_vol_path, nonlinear=True):
    ...
    return tuple(
        [
            ...
            for f in preprocess.antsCoReg(
                os.path.realpath(target_space_vol_path),
...
def _apply_warp_wrapper(
    s, vol, out, transforms, interp, type_code, dim, invert, project_root
):
    preprocess.antsApplyTransforms(
        s,
        vol,
        out,
        transforms,
        interp,
        img_type_code=type_code,
        dim=dim,
        invertTrans=invert,
    )
...
def apply_warp(
    ...
    args = [
        source,
        [vol_in_target_space] * len(source),
        [
            os.path.join(os.path.dirname(s), "reg_" + os.path.basename(s))
            for s in source
        ],
...
def apply_warp_4d(
    ...
    return apply_warp(
        source,
        vol_in_target_space,
        forward_gross_transform_path=forward_gross_transform_path,
        fine_transform_path=fine_transform_path,
        type_code=3,
        dim=3,
    )


def apply_warp_4d_refactor(
    source,
    vol_in_target_space,
    forward_gross_transform_path=None,
    fine_transform_path=None,
...
def apply_warp_inverse(
    ...
    if out is None:
        out = os.path.join(
            os.path.dirname(vol_in_target_space),
            "inverse_trans_" + os.path.basename(source),
        )
    inverse_transforms = [
        os.path.abspath(forward_gross_transform_path),
        os.path.abspath(reverse_fine_transform_path),
    ]
    to_invert = [True, False]
    preprocess.antsApplyTransforms(
        os.path.abspath(source),
        os.path.abspath(vol_in_target_space),
        os.path.abspath(out),
        inverse_transforms,
        interp,
        img_type_code=type_code,
        dim=dim,
        invertTrans=to_invert,
    )
    return os.path.relpath(out, project_root)


def apply_warp_inverse_vol_roi_dir(
    ds_vol_roi_dict,
    vol_in_target_space,
    forward_gross_transform_path,
    reverse_fine_transform_path,
    ...
    output_dir = preprocess._create_dir_if_needed(
...
def apply_binary_mask_vol(src_vol, mask):
    subj_root, project_root = _env_setup()
    out = os.path.join(
        os.path.dirname(src_vol),
        "masked_" + os.path.basename(src_vol).split(".")[0] + ".nii.gz",
    )
...
def create_slice_overlays(function_reg_vol, anatomical, reg_contrasts):
    ...
    if sig_sat < sig_thresh or sig_thresh < 0:
        raise ValueError(
            "Saturation point must be greater than threshold, threshold must be positive"
        )
...
def get_3d_rep(
    src: Union[List[str], str], use_topup=True, out_name="3d_epi_rep.nii.gz"
):
    """
    :param src:
    :return:
...
def itk_manual(source_vol, template):
    ...
    return tuple(
        [
            ...
            for p in preprocess._itkSnapManual(template, source_vol, out_dir)
        ]
    )
...
def _define_contrasts(condition_integerizers, base_index):
    ...
    print(
        "Recall: conditions are mapped as follows ",
        [
            "condition set " + str(i) + " : " + condition_integerizers[cond_int]
            for i, cond_int in enumerate(condition_integerizers)
        ],
    )
    add_contrast = True
    contrasts = []
...
def _order_def_from_nilearn_timing(condition_integerizer):
    files_dir = input_control.dir_input(
        "Enter directory containing all nilearn timing files. They will be assigned an"
        " order number."
    )
...
def _create_paradigm():
    ...
    para_dir = preprocess._create_dir_if_needed(
        os.path.join(subj_root, "../.."), "paradigms"
    )
    ...
    if name in proj_data["paradigms"] or os.path.exists(f_path):
        ...
        with open(f_path, "r") as f:
            ...
            config_file_path = os.path.relpath(
                os.path.join(para_dir, name + "_experiment_config.json"), project_root
            )
        ...
        print(
            "Successfully saved experiment / paradigm configuration json at",
            config_file_path,
        )
    ...
    predefine_orders = input_control.bool_input(
        "Predefine stimuli presentation orders? (highly recommended for your "
        "future convenience and reproducibility, unless your orders are in "
        "someway stochastic)"
    )
    para_def_dict["is_runtime_defined"] = not predefine_orders
    tr_len = input_control.numeric_input(
        "How many seconds per TR? (Enter -1 if can be variable.)"
    )
    if tr_len > 0:
        para_def_dict["lr_length"] = tr_len

    if predefine_orders:
        block_design = input_control.bool_input(
            "Define stimuli order using standard block definitions? Otherwise must "
    ...
    if predefine_orders:
        for order_set in range(num_order_sets):
            num_orders.append(
                int(
                    input(
                        "how many unique orders are there for condition set "
                        + str(order_set)
                        + "? "
                    )
                )
    ...
    for order_set in range(num_order_sets):
        num_conditions = int(
            input(
                "how many uniques conditions are there in condition set "
                + str(order_set)
                + "? "
            )
        )
        ...
        for i in range(num_conditions):
            temp_cond_map[str(i)] = input(
                "description for condition set"
                + str(order_set)
                + " condition #"
                + str(i)
            )
    ...
    if num_order_sets == 1:
        ...
        para_def_dict["condition_integerizer"] = para_def_dict["condition_integerizer"][
            0
        ]
    else:
    ...
    print(
        "Choose the base case condition from from primary condition set (condition set 0)"
    )
    para_def_dict["base_case_condition"] = int(
        input_control.select_option_input(list(condition_map.keys()))
    )
    ...
    if predefine_orders:
        ...
        if not block_design:
            print(
                "Since you're not generating order defs automatically, recommend loading from file"
            )
            load_orders_from_file = input_control.bool_input("Do you want to do that? ")
        if load_orders_from_file:
            print(
                "Please choose a tsv / csv file with order numbers as columns and a row for each tr, filled with "
            ...
            for order_set_idx, num_order in enumerate(num_orders):
                ...
                from_nilearn = input_control.bool_input(
                    "Load order defs from directory of nilearn timing files."
                )
                if from_nilearn:
                    para_def_dict["block_length_trs"] = 1
                    print(
                        "WARNING: set block length to 1 since nilearn orders are explicit"
                    )
            ...
            if len(num_orders) == 1:
                para_def_dict["order_number_definitions"] = para_def_dict[
                    "order_number_definitions"
                ][0]
        else:
            for order_set_idx, num_order in enumerate(num_orders):
                order_def_map = {}
                ...
                for i in range(num_order):
                    order_def_map[str(i)] = input_control.int_list_input(
                        "enter the block order if block design, or event sequence if event related for order number "
                        + str(i)
                    )
                para_def_dict["order_number_definitions"].append(order_def_map)
            if len(num_orders) == 1:
        ...
        print(
            "Stimuli orders will be defined at runtime. A mapping from the session name to the stimuli order list "
    ...
    while not good:
        try:
            contrasts_id, contrast_desc = _define_contrasts(
                para_def_dict["condition_integerizer"],
...
def create_load_ima_order_map(source):
    ...
    omap_path = os.path.relpath(
        os.path.join(sess_dir, "ima_order_map.json"), project_root
    )
    if os.path.exists(omap_path):
        if input_control.bool_input(
            "IMA -> order number map already defined for this session. Use existing?"
        ):
            return omap_path
    auto_fill = input_control.bool_input("fill with zeros? (use if runtime order def.)")
    ima_order_map = {}
    one_index_source = input_control.bool_input(
        "Are order numbers 1 indexed (in experiment logs)? "
    )
    for s in source:
...
def _design_matrices_from_condition_lists(
    ima_order_map,
    condition_names,
    num_conditions,
    base_conditions,
    ...
    print(
        "Stimuli orders are defined manually at runtime for this paradigm. "
        "Please load order csv file for session",
        sess_dir,
        "(rows are trs, cols are IMAs)",
    )
    ...
    for ima in ima_order_map.keys():
        ...
        if sess_name in paradigm_data["order_number_definitions"]:
            paradigm_data["order_number_definitions"][os.path.basename(sess_dir)][
                ima
            ] = clist
        else:
            paradigm_data["order_number_definitions"][os.path.basename(sess_dir)] = {
    ...
    for i, session_imas in enumerate(ima_order_map_path):
        ...
        if runtime_order_defs:
            ...
            for run_dir in source[i]:
                ...
                if is_block_design:
                    ...
                    sess_dm = analysis.design_matrix_from_run_list(
                        order,
                        num_conditions,
                        base_conditions,
                        condition_names,
        ...
        for j, sess_dm in enumerate(sess_dms):
            ...
            if os.path.exists(fix_path):
                ...
                print(
                    "WARN: No fixation data provided for session", sess_name, "ima", ima
                )
...
def get_beta_matrix(
    ...
    source = [
        [os.path.join(f, fname) if not os.path.isfile(f) else f for f in s]
        for s in source
    ]
    design_matrices, complete_source, tr_length = get_design_matrices(
        paradigm_path,
        ima_order_map_path,
        source,
        mion,
...
def create_contrast(beta_path, paradigm_path, use_cond_groups=True):
    ...
    if (
        "condition_groups" in paradigm_data
        and len(paradigm_data["condition_groups"]) > 0
        and use_cond_groups
    ):
        base_case = None
    ...
    contrast_paths = analysis.nilearn_contrasts(
        beta_path, contrast_def, contrast_descriptions, output_dir=sess_dir
    )
...
def construct_subject_glm(
    ...
    for key in sessions_dict:
        ...
        ima_order_maps.append(
            os.path.join(os.path.dirname(session_betas), "ima_order_map.json")
        )
        for ima in imas:
    ...
    print(
        "Constructed subject beta matrix for paradigm",
        para_name,
        "using",
        len(ima_order_maps),
        "sessions and",
...
def get_run_betas(para, mion=True):
    ...
    for key in sessions_dict:
        ...
        session_betas = sessions_dict[key]["beta_file"]
        imas = sessions_dict[key]["imas_used"]
        ima_order_map = os.path.join(
            os.path.dirname(session_betas), "ima_order_map.json"
        )
        for ima in imas:
            session_path = os.path.join(os.path.dirname(session_betas), str(ima))
        ...
        if behavior_key is not None:
            ...
            sess_behave = behavior_key.loc[
                (behavior_key["year"] == syear)
...
def surface_run_betas(data_key, ds_t1, t1, manual, auto, paradigm, white_surfs):
    ...
    for row in data.iterrows():
        ...
        try:
            ...
            surfs, _ = generate_subject_overlays(
                paradigm, reg_sources, white_surfs, t1, ds_t1
            )
...
def promote_session(reg_beta: str, paradigm_path, functional, ima_order_path, *argv):
    ...
    with open(proj_config_path, "r") as f:
        proj_config = json.load(f)
    session_net_path = os.path.relpath(
        os.path.join(subj_root, "sessions", session_id, "session_net.json"),
        project_root,
    )
...
def delete_paradigm(paradigm_path):
    ...
    with open(paradigm_path, "r") as f:
        paradigm_data = json.load(f)
    para_name = paradigm_data["name"]
    remove = input_control.bool_input(
        "WARNING: Paradigm "
        + para_name
...
def generate_subject_overlays(
    para_path, contrast_paths, white_surfs, t1_path, ds_t1_path
):
    """
    ...
    for contrast_rel_path in contrast_paths:
        ...
        for i, hemi in enumerate(["lh", "rh"]):
            ...
            print(subj_root)
            out_path = analysis.create_contrast_surface(
                os.path.abspath(white_surfs[i]),
...
def define_surface_rois(surface_overlays: list, surf_labels: dict):
    ...
    preprocess._create_dir_if_needed(
        os.path.join(subj_root, "rois", roi_set), "surf_labels"
    )
    print(
        "ROI labeling on surface must be done graphically in freeview. \n"
    ...
    surf_labels[roi_set] = os.path.relpath(
        os.path.join(subj_root, "rois", roi_set, "surf_labels"), project_root
    )
    input("done? ")
    return surf_labels
...
def manual_volume_rois(ds_t1: str, paradigm_complete: bool, ds_vol_rois: dict):
    ...
    if paradigm_complete is False:
        print(
            "Please create subject contrasts for at least one paradigm before defining functional ROIs."
        )
        return None
    subj_root, project_root = _env_setup()
    preprocess._create_dir_if_needed(subj_root, "rois")
    ...
    preprocess._create_dir_if_needed(os.path.join(subj_root, "rois"), roi_set)
    preprocess._create_dir_if_needed(
        os.path.join(subj_root, "rois", roi_set), "ds_vol_rois"
    )
    print(
        "Manual ROI labeling on volumes must be done graphically in freeview. \n"
    ...
    ds_vol_rois[roi_set] = os.path.relpath(
        os.path.join(subj_root, "rois", roi_set, "ds_vol_rois"), project_root
    )
    input("done? ")
    return ds_vol_rois
...
def automatic_volume_rois(paradigm_file, paradigm_contrasts: List[str], ds_vol_rois):
    ...
    out_dir = preprocess._create_dir_if_needed(
        os.path.join(subj_root, "rois", roi_set_name), "ds_vol_rois"
    )
    contrasts_touse = []
    ...
    out_dir = analysis.get_auto_roi_masks(
        contrasts_touse, out_dir, max_rois=max_rois, min_rois=min_rois
    )
    ds_vol_rois[roi_set_name] = os.path.relpath(out_dir, project_root)
    return ds_vol_rois


def surf_labels_to_vol_mask(
    surf_labels: dict, white_surface, t1, ds_t1, vol_rois: dict
) -> dict:
    """
    ...
    hemis = ["lh", "rh"]
    out_dir = preprocess._create_dir_if_needed(
        os.path.join(subj_root, "rois", roi_set_name), "vol_rois"
    )
    for hemi in hemis:
...
def load_white_surfs():
    ...
    for hemi in ["left", "right"]:
        ...
        surf_proj_path = os.path.relpath(
            os.path.join(subj_root, "surf", surf_name), project_root
        )
...
def motion_correction_wrapper(
    source, targets, moco_is_nonlinear=None, fname="f_topup_sphinx.nii.gz"
):
    """
    :param source:
    :param targets:
    ...
    for i, t in enumerate(targets):
        preprocess.motion_correction(
            source, t, check_rms=False, fname=fname, outname="temp_moco.nii.gz"
        )
        disp = 0
        for run_dir in source:
            disp_file = os.path.join(
                os.path.dirname(run_dir), "temp_moco.nii.gz_abs.rms"
            )
...
def topup_wrapper(functional_dirs, use_topup, fname="f_nordic.nii"):
    if use_topup:
        in_paths = [
            os.path.join(f, fname) if not os.path.isfile(f) else f
            for f in functional_dirs
        ]
        image_1_path = input_control.dir_input(
            "Enter the path to the first spin echo image."
        )
        image_1_enc = input_control.str_list_input(
            "What phase encoding is this image? (HF, FH, RL, LR)"
        )[0]
        image_2_path = input_control.dir_input(
            "Enter the path to the second spin echo image."
        )
        image_2_enc = input_control.str_list_input(
            "What phase encoding is this image? (HF, FH, RL, LR)"
        )[0]
        number_images = input_control.int_input(
            "How many images do you want to use topup with? (Recommended 2-3)"
        )
        func_enc = input_control.str_list_input(
            "What phase encoding are the functional images in? (HF, FH, RL, LR)"
        )[0]
        out = preprocess.topup(
            in_paths,
            image_1_path,
            image_2_path,
            image_1_enc,
...
def time_series_order_vs_all_functional(
    functional_dirs,
    ima_order_data,
    paradigm_data,
    target_condition,
    ...
    output = os.path.relpath(
        os.path.join(
            ...
            para_name
            + "_condition_"
            + conditions[str(target_condition)]
            + "_timecourse_comparison.nii",
        ),
        project_root,
    )
    return analysis.get_condition_time_series_comparision(
        functional_dirs,
        block_length,
        ima_order_data,
        order_def,
        ...
        output,
        fname=fname,
        pre_onset_trs=pre_onset_trs,
        post_offset_trs=post_offset_trs,
    )
...
def get_vol_rois_time_series(vol_rois: dict, ts_dict: dict, ds_t1_path):
    ...
    ts_dir = preprocess._create_dir_if_needed(
        os.path.join(subj_root, "rois", roi_set_name), "time_series"
    )
    ...
    for condition in coi:
        for session_id in sessions_dict:
            ...
            functional_dirs = [
                os.path.join(sess_dir, str(ima))
                for ima in sessions_dict[session_id]["imas_used"]
            ]
            ts_func_path = time_series_order_vs_all_functional(
                functional_dirs,
                ima_order_data,
                paradigm_data,
                condition,
            ...
            ts_func_path = apply_warp(
                ts_func_path,
                ds_t1_path,
                manual_transform,
                fine_forward_transform,
                type_code=3,
                dim=3,
            )
            ...
            for i, roi_path in enumerate(roi_paths):
                ...
                ts, _, colors = analysis._get_roi_time_course(
                    roi_data,
                    1,
                    ts_data,
                    axs[i],
        ...
        for i, roi_path in enumerate(roi_paths):
            ...
            ts_path = os.path.join(
                ts_dir, roi_name + "_".join(cond_descs) + "_condition_ts.npy"
            )
...
def binary_masks_from_int_atlas(
    atlas: str,
    desired_indices: Union[None, List[Union[int, Tuple[int]]]] = None,
    index_names=None,
    out_dir=None,
):
    """
    ...
    if index_names is not None and len(index_names) != len(desired_indices):
        raise ValueError(
            "if names are provided must be same length as desired index list"
        )
    for i, idx in enumerate(desired_indices):
        ...
        mask_nii = nibabel.Nifti1Image(
            mask, header=atlas_nii.header, affine=atlas_nii.affine
        )
