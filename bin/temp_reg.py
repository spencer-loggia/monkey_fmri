...
if __name__=='__main__':
    ...
    def temp_antsCoreg(fixedP, movingP, outdir, radiusi1, radiusi2, updateFV, totalFV, outToken, nonlinear=True):
        cur_dir = os.getcwd()
        os.chdir(outdir)
        print(outdir)
        ...
        cmd = "antsRegistration" \
              ...
              " --output [ ./,./Warped_"+outToken+".nii.gz,./InverseWarped_"+outToken+".nii.gz ] " \
        ...
        if nonlinear:
            cmd = cmd + " --transform SyN[ .01," + updateFV + "," + totalFV +" ] --metric CC[ " + fixedP + "," + movingP + ",1," + radiusi1 + " ]" \
              " --convergence [ 300x300,1e-9,25 ] --shrink-factors 2x1 --smoothing-sigmas 1x0vox"\
              " --transform SyN[ .01," + updateFV + "," + totalFV +" ] --metric CC[ " + fixedP + "," + movingP + ",1," + radiusi2 + " ]" \
    ...
    temp_antsCoreg(fixedP=fixedP, movingP=movingP, outdir=outdir, radiusi1=radiusi1, radiusi2=radiusi2, updateFV=updateFV, totalFV=totalFV, outToken=outToken, nonlinear=True)
